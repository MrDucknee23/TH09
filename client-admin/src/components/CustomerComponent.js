...
class Customer extends Component {
  ...
  apiGetOrdersByCustID(cid) {
    ...
    axios.get('/api/admin/orders/customer/' + cid, config).then((res) => {
      ...
      console.error('API GET ORDERS BY CUSTOMER ID ERROR:', err);
  ...
  apiPutCustomerDeactive(id, token) {
    ...
    axios.put('/api/admin/customers/deactive/' + id, body, config).then((res) => {
      const result = res.data;
      // if result is success object or updated object
      if (result && (result.success === true || result.active === 0 || result._id)) {
        this.apiGetCustomers();
      } else {
        alert('SORRY BABY !');
      }
    }).catch((err) => {
      console.error('API PUT CUSTOMER DEACTIVE ERROR:', err);
      alert('SORRY BABY !');
    });
  }
  ...
  apiGetCustomerSendmail(id) {
    ...
    axios.get('/api/admin/customers/sendmail/' + id, config).then((res) => {
      ...
      console.error('API GET CUSTOMER SENDMAIL ERROR:', err);
